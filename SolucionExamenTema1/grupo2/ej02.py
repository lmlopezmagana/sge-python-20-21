...
def media_dia_semana(data):
    aux = {}
    for fecha, precipitacion in data.items():
        ds = dia_semana(fecha)
        if ds not in aux.keys():
            aux[ds] = [precipitacion]
        else:
            aux[ds].append(precipitacion)

    result = {}

    for ds in aux.keys():
        acumulador = 0
        for precipitacion in aux[ds]:
            acumulador += precipitacion
        result[ds] = acumulador / len(aux[ds])

    return result


print(media_dia_semana(datos))

def menores(data):
    ultimo, penultimo, antepenultimo = 10000, 10000, 10000
    fecha_ultimo, fecha_penultimo, fecha_antepenultimo = '','',''

    for fecha, precipitacion in data.items():
        if precipitacion > 0:
            if precipitacion < ultimo:
                antepenultimo = penultimo
                fecha_antepenultimo = fecha_penultimo
                penultimo = ultimo
                fecha_penultimo = fecha_ultimo
                ultimo = precipitacion
                fecha_ultimo = fecha
            elif precipitacion < penultimo:
                antepenultimo = penultimo
                fecha_antepenultimo = fecha_penultimo
                penultimo = precipitacion
                fecha_penultimo = fecha
            elif precipitacion < antepenultimo:
                antepenultimo = precipitacion
                fecha_antepenultimo = fecha

    result = {}
    result[fecha_ultimo] = ultimo
    result[fecha_penultimo] = penultimo
    result[fecha_antepenultimo] = antepenultimo
